...
def expample1():
    ...
    for start_x, start_y, end_x, end_y, _ in lines:
        if np.isnan(start_x) or np.isnan(start_y) or np.isnan(end_x) or np.isnan(end_y):
            print("警告: 发现 NaN 值，跳过绘制直线")
        else:
            # 所有值都有效，可以安全地绘制直线
            cv2.line(out, (int(np.round(start_x)), int(np.round(start_y))),
                    (int(np.round(end_x)), int(np.round(end_y))), (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
